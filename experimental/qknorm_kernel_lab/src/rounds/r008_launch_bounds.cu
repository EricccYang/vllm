...
// R008: __launch_bounds__ for 1-head kernel — already uses 27 regs so this is mainly declarative.
template <typename scalar_t_in, typename scalar_t_cache, int head_dim,
          bool interleave>
__global__ void __launch_bounds__(256, 8) fusedQKNormRopeImproveKernel(
...
// R008: __launch_bounds__ constrains registers for higher occupancy.
// 256 threads/block, target min 8 blocks/SM → compiler uses ≤32 regs.
template <typename scalar_t_in, typename scalar_t_cache, int head_dim,
          bool interleave, int HEADS_PER_WARP>
__global__ void __launch_bounds__(256, 8) fusedQKNormRopeImproveKernelNTokenHeads(
    void* qkv_void,
