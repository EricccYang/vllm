...
// R005: On-the-fly RoPE frequency computation (from compute_kernel variant).
__device__ __forceinline__ float r005_rope_freq(int half_dim, int rotary_dim, float rope_base) {
  return powf(rope_base,
...
__global__ void fusedQKNormRopeImproveKernelNTokenHeads(
  ...
  // R005: No cos/sin smem region. Only QKV tiles in smem.

  T_in* qkv = reinterpret_cast<T_in*>(qkv_void);
  T_in const* q_weight = reinterpret_cast<T_in const*>(q_weight_void);
  T_in const* k_weight = reinterpret_cast<T_in const*>(k_weight_void);
  // cos_sin_cache_void is accepted but not used (computed on-the-fly).
  (void)cos_sin_cache_void;
  ...
  // R005: QKV smem only — no cos/sin region.
  ...
  rope_cp_async_fence();  // commit group 0 (QKV only)

  // R005: Compute cos/sin on-the-fly into register arrays while QKV loads.
...
#pragma unroll
      for (int i = 0; i < numElemsPerThread / 2; ++i) {
        ...
        int const half_dim = dim_idx / 2;
        float const freq = r005_rope_freq(half_dim, rotary_dim, ROPE_BASE);
...
#pragma unroll
  ...
  // Wait for QKV async copy to complete.
...
#pragma unroll
    ...
    // R005: No group 1 wait needed — cos/sin computed on-the-fly.
...
#pragma unroll
        for (int i = 0; i < numElemsPerThread / 2; ++i) {
          ...
          float const val1 = elements[idx1];
          int const half_dim = dim_idx / 2;
          // R005: cos/sin from precomputed registers.
          elements[idx0] = val0 * cos_reg[i] - val1 * sin_reg[i];
          elements[idx1] = val0 * sin_reg[i] + val1 * cos_reg[i];
        }
      } else {
        __syncwarp();
...
#pragma unroll
        for (int i = 0; i < numElemsPerThread; i++) {
          ...
          // R005: cos/sin from precomputed registers.
          elements[i] = elements[i] * cos_reg[i] + elements2[i] * sin_reg[i];
        }
        __syncwarp();
      }
    }
...
void launchFusedQKNormRopeImproveNTokenHeads(
  ...
  // R005: No cos/sin smem — only QKV tiles.
