...
#pragma unroll
    ...
    // R003: elements2 removed — inline temp used instead.
...
#pragma unroll
        for (int i = 0; i < numElemsPerThread; i++) {
          ...
          elements[i] = elements[i] * cos_val + elem2 * sin_val;
        }
...
__global__ void fusedQKNormRopeImproveKernelNTokenHeads(
  ...
  float elements[numElemsPerThread];
  // R003: elements2 removed — use inline temp to reduce register pressure.
