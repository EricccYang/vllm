...
#pragma unroll
    ...
    // R002: Vectorized weight load for 1-head kernel.
    {
      int const woff = laneId * numElemsPerThread;
      vec_T w_vec = isQ
          ? *reinterpret_cast<vec_T const*>(&q_weight[woff])
          : *reinterpret_cast<vec_T const*>(&k_weight[woff]);
      constexpr int npk = elemSizeBytes / sizeof(T2_in);
#pragma unroll
      for (int i = 0; i < npk; ++i) {
        float2 wv = Converter::convert(*(reinterpret_cast<T2_in const*>(&w_vec) + i));
        elements[2*i]   *= rms_rcp * wv.x;
        elements[2*i+1] *= rms_rcp * wv.y;
      }
    }
...
__global__ void fusedQKNormRopeImproveKernelNTokenHeads(
  ...
  // R002: Vectorized weight preload — one vec load instead of N scalar loads.
  float q_w[numElemsPerThread];
  float k_w[numElemsPerThread];
  {
    int const woff = laneId * numElemsPerThread;
    vec_T q_vec = *reinterpret_cast<vec_T const*>(&q_weight[woff]);
    vec_T k_vec = *reinterpret_cast<vec_T const*>(&k_weight[woff]);
    constexpr int npk = elemSizeBytes / sizeof(T2_in);
#pragma unroll
    for (int i = 0; i < npk; ++i) {
      float2 qv = Converter::convert(*(reinterpret_cast<T2_in const*>(&q_vec) + i));
      float2 kv = Converter::convert(*(reinterpret_cast<T2_in const*>(&k_vec) + i));
      q_w[2*i]   = qv.x;  q_w[2*i+1] = qv.y;
      k_w[2*i]   = kv.x;  k_w[2*i+1] = kv.y;
    }
  }
