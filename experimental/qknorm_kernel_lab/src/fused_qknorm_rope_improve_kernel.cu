...
namespace tensorrt_llm::common_improve {
...
namespace tensorrt_llm::tuning_improve {

constexpr int kAnyArch = -1;
constexpr int kAnyHeadDim = -1;

struct QKNormRopeRange {
  int min_inclusive;
  int max_inclusive;

  inline __host__ __device__ bool contains(int value) const {
    return min_inclusive <= value && value <= max_inclusive;
  }
};

struct QKNormRopeDispatchKey {
  int sm_version;
  int num_tokens;
  int total_qk_heads;
  int head_dim;

  inline __host__ __device__ int total_qk_units() const {
    return num_tokens * total_qk_heads;
  }
};

struct QKNormRopeLaunchConfig {
  int block_size;
  int token_heads_per_warp;

  inline __host__ __device__ bool is_valid() const {
    bool valid_block = block_size == 128 || block_size == 256 || block_size == 512;
    bool valid_heads = token_heads_per_warp == 2 || token_heads_per_warp == 4 ||
                       token_heads_per_warp == 8;
    return valid_block && valid_heads;
  }
};

struct QKNormRopeTuningEntry {
  int sm_version;
  int head_dim;
  QKNormRopeRange num_tokens;
  QKNormRopeRange total_qk_heads;
  QKNormRopeLaunchConfig config;

  inline __host__ __device__ bool matches(
      QKNormRopeDispatchKey const& key) const {
    bool arch_match = sm_version == kAnyArch || sm_version == key.sm_version;
    bool head_dim_match = head_dim == kAnyHeadDim || head_dim == key.head_dim;
    return arch_match && head_dim_match &&
           num_tokens.contains(key.num_tokens) &&
           total_qk_heads.contains(key.total_qk_heads);
  }
};

struct QKNormRopeLookupResult {
  bool found;
  int matched_index;
  QKNormRopeLaunchConfig config;
};

inline __host__ __device__ QKNormRopeLaunchConfig makeLaunchConfig(
    int block_size, int token_heads_per_warp) {
  return QKNormRopeLaunchConfig{block_size, token_heads_per_warp};
}

template <size_t N>
inline QKNormRopeLookupResult lookupLaunchConfig(
    QKNormRopeDispatchKey const& key,
    QKNormRopeTuningEntry const (&table)[N]) {
  for (size_t idx = 0; idx < N; ++idx) {
    if (table[idx].matches(key)) {
      return QKNormRopeLookupResult{
          true, static_cast<int>(idx), table[idx].config};
    }
  }
  return QKNormRopeLookupResult{
      false, -1, QKNormRopeLaunchConfig{0, 0}};
}
...
// cp.async.ca: cache at (L1+L2), supports 4/8/16 bytes for sub-16B loads
...
// Variant: one warp processes N token-heads (N = 2, 4, etc.; for performance comparison).


// Variant for one warp / one token-head. Copy of fusedQKNormRopeKernel.
...
__global__ void fusedQKNormRopeImproveKernelNTokenHeads(
    void* qkv_void,
  ...
  // Region 1: cos/sin only (do not use for head data).
  ...
  // This warp handles one token's head chunk: same token for all heads → reuse cos/sin.
  ...
  int const num_heads_this_warp = (first_head + HEADS_PER_WARP <= total_qk_heads)
                                      ? HEADS_PER_WARP
  ...
  using vec_T =
      typename tensorrt_llm::common_improve::packed_as<uint, vecSize>::type;

  // QKV smem region: layout per-warp, HEADS_PER_WARP tiles × (32 × elemSizeBytes) bytes.
  ...
  // === Group 0: async load all heads' QKV into smem (issued first → older group). ===
  for (int k = 0; k < num_heads_this_warp; ++k) {
    ...
    char* smem_dst = this_warp_head_smem + k * qkv_tile_bytes + laneId * elemSizeBytes;
    rope_cp_async_ca(smem_dst,
                     reinterpret_cast<const char*>(&qkv[offThread]),
                     elemSizeBytes);
  }
  rope_cp_async_fence();  // commit group 0 (QKV, older)

  // === Group 1: async load cos/sin into smem (issued second → newer group). ===
  ...
  for (int copyId = laneId; copyId < num_copies; copyId += 32) {
    ...
    const char* glob_ptr = reinterpret_cast<const char*>(cache_ptr) + copyId * 16;
    rope_cp_async4(smem_ptr, glob_ptr);
  }
  rope_cp_async_fence();  // commit group 1 (cos/sin, newer)

  // wait<1>: at most 1 group (group 1, cos/sin) may still be pending → group 0 (QKV) is done.
  ...
  // Preload q_weight and k_weight into registers (same for all heads, only load once).
...
#pragma unroll
  ...
  for (int k = 0; k < num_heads_this_warp; ++k) {
    ...
    if (isQ) {
      offsetWarp =
          tokenIdx * num_heads * head_dim + headIdx * head_dim;
    } else {
      offsetWarp = tokenIdx * num_heads * head_dim + num_heads_q * head_dim +
                   headIdx * head_dim;
    }
    ...
    // === Part 1: QK Norm (read from smem, group 0 already waited above). ===
    float sumOfSquares = 0.0f;
    {
      char const* smem_src =
...
#pragma unroll
    ...
    // wait<0>: first iteration only – ensure group 1 (cos/sin) is done before RoPE.
    if (k == 0) rope_cp_async_wait<0>();

    // === Part 2: RoPE using cos/sin from shared (no global load).
    if (laneId < rotary_lanes) {
      if constexpr (interleave) {
#pragma unroll
        for (int i = 0; i < numElemsPerThread / 2; ++i) {
          ...
          int const half_dim = dim_idx / 2;
          float const cos_val =
              CacheConverter::convert(cos_smem[half_dim]);
          float const sin_val =
              CacheConverter::convert(sin_smem[half_dim]);
          elements[idx0] = val0 * cos_val - val1 * sin_val;
          elements[idx1] = val0 * sin_val + val1 * cos_val;
        }
      } else {
...
// Ablation variant: NTokenHeads WITHOUT cos/sin cp.async to smem.
// cos/sin loaded from global memory via VLLM_LDG each iteration (like base 1h).
// QKV cp.async to smem is kept. Only the cos/sin path is reverted.
template <typename scalar_t_in, typename scalar_t_cache, int head_dim,
          bool interleave, int HEADS_PER_WARP>
__global__ void fusedQKNormRopeNTokenHeadsNoCossinAsync(
  ...
  extern __shared__ char smem_storage[];
  // No cos/sin smem region — QKV tiles start at offset 0.
  ...
  // QKV smem only (no cos/sin region).
  int const qkv_tile_bytes = 32 * elemSizeBytes;
  char* const this_warp_head_smem =
      smem_storage + warpId * (HEADS_PER_WARP * qkv_tile_bytes);

  // === Group 0: async load all heads' QKV into smem. ===
  for (int k = 0; k < num_heads_this_warp; ++k) {
    int const localHeadIdx = first_head + k;
    bool const isQ = localHeadIdx < num_heads_q;
    int const headIdx = isQ ? localHeadIdx : localHeadIdx - num_heads_q;
  ...
  rope_cp_async_fence();
  // Only one group — wait for it.
  rope_cp_async_wait<0>();

  float elements[numElemsPerThread];
  float elements2[numElemsPerThread];
  int const rotary_lanes = rotary_dim / numElemsPerThread;

  // cos/sin pointers from GLOBAL memory (like base 1h kernel).
  int64_t const pos_id = position_ids[tokenIdx];
  T_cache const* const cache_ptr = cos_sin_cache + pos_id * rotary_dim;
  int const embed_dim = rotary_dim / 2;
...
#pragma unroll
    ...
    // === Part 2: RoPE — cos/sin from GLOBAL memory (VLLM_LDG). ===
    if (laneId < rotary_lanes) {
      if constexpr (interleave) {
#pragma unroll
        for (int i = 0; i < numElemsPerThread / 2; ++i) {
...
void launchFusedQKNormRopeImprove(void* qkv, int const num_tokens,
  ...
  int const gridSize = tensorrt_llm::common_improve::divUp(totalWarps, warpsPerBlock);
  dim3 gridDim(gridSize);
  dim3 blockDim(block_size);
  VLLM_NVTX_RANGE_PUSH("FusedQKNormRopeImprove");
  switch (head_dim) {
    case 64:
      DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
        fusedQKNormRopeImproveKernel<scalar_t_in, scalar_t_cache, 64, INTERLEAVE>
            <<<gridDim, blockDim, 0, stream>>>(qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight, k_weight, cos_sin_cache, position_ids, num_tokens, rotary_dim);
      });
      break;
    case 128:
      DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
        fusedQKNormRopeImproveKernel<scalar_t_in, scalar_t_cache, 128, INTERLEAVE>
            <<<gridDim, blockDim, 0, stream>>>(qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight, k_weight, cos_sin_cache, position_ids, num_tokens, rotary_dim);
      });
      break;
    case 256:
      DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
        fusedQKNormRopeImproveKernel<scalar_t_in, scalar_t_cache, 256, INTERLEAVE>
            <<<gridDim, blockDim, 0, stream>>>(qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight, k_weight, cos_sin_cache, position_ids, num_tokens, rotary_dim);
      });
      break;
    default:
      TORCH_CHECK(false, "Unsupported head dimension for fusedQKNormRopeImprove: ", head_dim);
  }
  VLLM_NVTX_RANGE_POP();
}

// Launch: one warp processes N token-heads (N = 2, 4, etc.).
template <typename scalar_t_in, typename scalar_t_cache>
void launchFusedQKNormRopeImproveNTokenHeads(
  ...
  TORCH_CHECK(token_heads_per_warp == 1 || token_heads_per_warp == 2 ||
              ...
              "token_heads_per_warp must be 1, 2, 4, or 8, got ", token_heads_per_warp);

  // token_heads_per_warp == 1: delegate to the 1h baseline kernel
  if (token_heads_per_warp == 1) {
    launchFusedQKNormRopeImprove<scalar_t_in, scalar_t_cache>(
        qkv, num_tokens, num_heads_q, num_heads_k, num_heads_v, head_dim,
        rotary_dim, eps, q_weight, k_weight, cos_sin_cache, interleave,
        position_ids, block_size, stream);
    return;
  }
  ...
  int const gridSize =
      tensorrt_llm::common_improve::divUp(total_warps, warpsPerBlock);
  dim3 gridDim(gridSize);
  dim3 blockDim(block_size);
  ...
  // QKV smem: HEADS_PER_WARP tiles per warp, each tile 32 * (head_dim/32*2) = 2*head_dim bytes.
  size_t const qkv_smem_per_warp =
      static_cast<size_t>(token_heads_per_warp) * 2u * static_cast<size_t>(head_dim);
  size_t const smem_bytes =
      warpsPerBlock * static_cast<size_t>(rotary_dim) * cache_elem_size +
      warpsPerBlock * qkv_smem_per_warp;
  VLLM_NVTX_RANGE_PUSH("FusedQKNormRopeImproveNTokenHeads");
#define LAUNCH_N_TOKEN_HEADS(N)                                              \
  do {                                                                       \
    switch (head_dim) {                                                      \
      case 64:                                                               \
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                        \
          fusedQKNormRopeImproveKernelNTokenHeads<scalar_t_in, scalar_t_cache, 64, INTERLEAVE, (N)> \
      ...
      case 128:                                                              \
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                        \
          fusedQKNormRopeImproveKernelNTokenHeads<scalar_t_in, scalar_t_cache, 128, INTERLEAVE, (N)> \
              <<<gridDim, blockDim, smem_bytes, stream>>>(                    \
                  qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight, \
                  k_weight, cos_sin_cache, position_ids, num_tokens,         \
                  rotary_dim);                                               \
      ...
      case 256:                                                              \
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                        \
          fusedQKNormRopeImproveKernelNTokenHeads<scalar_t_in, scalar_t_cache, 256, INTERLEAVE, (N)> \
              <<<gridDim, blockDim, smem_bytes, stream>>>(                    \
                  qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight, \
                  k_weight, cos_sin_cache, position_ids, num_tokens,         \
                  rotary_dim);                                               \
...
// Launch: NTokenHeads variant WITHOUT cos/sin cp.async (ablation study).
template <typename scalar_t_in, typename scalar_t_cache>
void launchFusedQKNormRopeNTokenHeadsNoCossinAsync(
    void* qkv, int const num_tokens, int const num_heads_q,
    int const num_heads_k, int const num_heads_v, int const head_dim,
    int const rotary_dim, float const eps, void const* q_weight,
    void const* k_weight, void const* cos_sin_cache, bool const interleave,
  ...
  TORCH_CHECK(token_heads_per_warp == 2 || token_heads_per_warp == 4 ||
                  token_heads_per_warp == 8,
              "token_heads_per_warp must be 2, 4, or 8, got ", token_heads_per_warp);
  ...
  size_t const qkv_smem_per_warp =
      static_cast<size_t>(token_heads_per_warp) * 2u * static_cast<size_t>(head_dim);
  size_t const smem_bytes = warpsPerBlock * qkv_smem_per_warp;

#define LAUNCH_NO_COSSIN(N)                                                   \
  do {                                                                        \
    switch (head_dim) {                                                       \
      case 64:                                                                \
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                         \
          fusedQKNormRopeNTokenHeadsNoCossinAsync<scalar_t_in, scalar_t_cache, 64, INTERLEAVE, (N)> \
              <<<gridDim, blockDim, smem_bytes, stream>>>(                     \
                  qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight,  \
                  k_weight, cos_sin_cache, position_ids, num_tokens,          \
                  rotary_dim);                                                \
      ...
      case 128:                                                               \
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                         \
          fusedQKNormRopeNTokenHeadsNoCossinAsync<scalar_t_in, scalar_t_cache, 128, INTERLEAVE, (N)> \
              <<<gridDim, blockDim, smem_bytes, stream>>>(                     \
                  qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight,  \
                  k_weight, cos_sin_cache, position_ids, num_tokens,          \
                  rotary_dim);                                                \
      ...
      case 256:                                                               \
        DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {                         \
          fusedQKNormRopeNTokenHeadsNoCossinAsync<scalar_t_in, scalar_t_cache, 256, INTERLEAVE, (N)> \
              <<<gridDim, blockDim, smem_bytes, stream>>>(                     \
                  qkv, num_heads_q, num_heads_k, num_heads_v, eps, q_weight,  \
                  k_weight, cos_sin_cache, position_ids, num_tokens,          \
                  rotary_dim);                                                \
...
#undef LAUNCH_NO_COSSIN
}

}  // namespace tensorrt_llm::kernels

void fused_qk_norm_rope_improve(
    torch::Tensor& qkv,
    int64_t num_heads_q,
    int64_t num_heads_k,
    int64_t num_heads_v,
  ...
  VLLM_DISPATCH_HALF_TYPES(qkv.scalar_type(), "fused_qk_norm_rope_improve_kernel", [&] {
    using qkv_scalar_t = scalar_t;
    VLLM_DISPATCH_FLOATING_TYPES(
        cos_sin_cache.scalar_type(), "fused_qk_norm_rope_improve_kernel", [&] {
          using cache_scalar_t = scalar_t;
          tensorrt_llm::kernels::launchFusedQKNormRopeImprove<qkv_scalar_t,
                                                             cache_scalar_t>(
...
void fused_qk_norm_rope_improve_2_token_heads(
  ...
  // (benchmark: bench_token_heads_crossover.py on H100, num_heads_q=32, num_heads_k=8)
  {
  ...
  VLLM_DISPATCH_HALF_TYPES(qkv.scalar_type(), "fused_qk_norm_rope_improve_n_token_heads_kernel", [&] {
    using qkv_scalar_t = scalar_t;
    VLLM_DISPATCH_FLOATING_TYPES(
        cos_sin_cache.scalar_type(), "fused_qk_norm_rope_improve_n_token_heads_kernel", [&] {
          using cache_scalar_t = scalar_t;
          tensorrt_llm::kernels::launchFusedQKNormRopeImproveNTokenHeads<
              ...
              cos_sin_cache.data_ptr(), !is_neox,
              reinterpret_cast<int64_t const*>(position_ids.data_ptr()),
              static_cast<int>(block_size), static_cast<int>(token_heads_per_warp), stream);
        });
  });
}
...
void fused_qk_norm_rope_improve_force_hpw(
  ...
  TORCH_CHECK(qkv.dim() == 2, "QKV tensor must be 2D");
  TORCH_CHECK(position_ids.dim() == 1, "Position IDs must be 1D");
  TORCH_CHECK(q_weight.size(0) == head_dim);
  TORCH_CHECK(k_weight.size(0) == head_dim);
  TORCH_CHECK(cos_sin_cache.size(1) % 2 == 0, "rotary_dim must be even");
  TORCH_CHECK(cos_sin_cache.size(1) <= head_dim);
  ...
  VLLM_DISPATCH_HALF_TYPES(qkv.scalar_type(), "force_hpw_kernel", [&] {
    using qkv_scalar_t = scalar_t;
    VLLM_DISPATCH_FLOATING_TYPES(
        cos_sin_cache.scalar_type(), "force_hpw_kernel", [&] {
          using cache_scalar_t = scalar_t;
          tensorrt_llm::kernels::launchFusedQKNormRopeImproveNTokenHeads<
              qkv_scalar_t, cache_scalar_t>(
              qkv.data_ptr(), static_cast<int>(num_tokens),
              ...
              static_cast<int>(token_heads_per_warp), stream);
        });
  });
}
...
void fused_qk_norm_rope_no_cossin_async(
  ...
  VLLM_DISPATCH_HALF_TYPES(qkv.scalar_type(), "no_cossin_async_kernel", [&] {
    using qkv_scalar_t = scalar_t;
    VLLM_DISPATCH_FLOATING_TYPES(
        cos_sin_cache.scalar_type(), "no_cossin_async_kernel", [&] {
          using cache_scalar_t = scalar_t;
          tensorrt_llm::kernels::launchFusedQKNormRopeNTokenHeadsNoCossinAsync<
              qkv_scalar_t, cache_scalar_t>(
              qkv.data_ptr(), static_cast<int>(num_tokens),
              static_cast<int>(num_heads_q), static_cast<int>(num_heads_k),
              static_cast<int>(num_heads_v), static_cast<int>(head_dim),
